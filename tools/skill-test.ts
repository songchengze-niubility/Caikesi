...
// 确定性面板：攻方概率/加成全清零。被打的怪自带 dodgeRate(如僵尸 5%)，
// 用 pinDeterministic 换引用清零（不污染全局配置对象）。
function detStats(p: Partial<CombatStats>): CombatStats {
...
test('BattleManager 集成：效果列表技能——毒箭一击挂上中毒并周期跳伤', () => {
    ...
    const mgr = new BattleManager(470, 836, 0, {
        dps: detStats({ hp: 99999, atk: 30, range: 0 }),
    }, ['dps']);
...
test('BattleManager 集成：zone 投递——释放生成毒池，敌人周期扣血，事件 hits 为空', () => {
    const mgr = new BattleManager(470, 836, 0, {
        dps: detStats({ hp: 99999, atk: 50, range: 0 }),
    }, ['dps']);
    mgr.soldiers[0].skills = new UnitSkills([mkDef({
        trigger: 'timer', triggerValue: 0.1, target: 'nearest', maxTargets: 1,
        effects: [{ kind: 'damage', mult: 0.2 }],
        delivery: { kind: 'zone', radius: 150, duration: 5, period: 1 },
    })]);
    for (let i = 0; i < 10 && mgr.enemies.length === 0; i++) mgr.tick(0.05);
    const e = mgr.enemies[0];
    pinDeterministic(e);
    ...
    const events: import('../assets/scripts/combat/BattleManager').BattleEvent[] = [];
    for (let i = 0; i < 4 && mgr.zones.length === 0; i++) { mgr.tick(0.05); events.push(...mgr.drainEvents()); }
    assert.equal(mgr.zones.length, 1, '释放应生成 zone');
    const cast = events.find(ev => ev.type === 'skillCast');
    assert.ok(cast && cast.type === 'skillCast' && cast.hits.length === 0, '投递技能事件 hits 为空');
    assert.ok(events.some(ev => ev.type === 'zoneSpawned'));
    mgr.soldiers[0].skills = new UnitSkills([]);   // 卸掉技能隔离毒池
    const hp0 = e.hp;
    for (let i = 0; i < 21; i++) mgr.tick(0.05);   // 1.05s → 1 跳
    const perTick = Math.max(1, Math.round(Math.max(50 - e.stats.def, 50 * BattleConfig.combat.minDamageRate) * 0.2));
    assert.equal(e.hp, hp0 - perTick, '毒池周期扣血');
});
