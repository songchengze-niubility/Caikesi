...
test('ChestConfig.rewards：三种箱型奖励档案由 chest.xlsx 驱动（表化取代硬编码）', () => {
    ...
    // 2026-07-12 宝石砍档：普通箱 0 颗、Boss 2 颗、章节 3 颗（宝石成 Boss 专属惊喜）
    assert.deepEqual(r.normal, { equipmentRolls: 1, forgeStoneMin: 2, forgeStoneMax: 4, gemCount: 0, gemLevelMin: 1, gemLevelMax: 1, scrollMin: 0, scrollMax: 0 });
    assert.deepEqual(r.boss, { equipmentRolls: 2, forgeStoneMin: 4, forgeStoneMax: 8, gemCount: 2, gemLevelMin: 1, gemLevelMax: 2, scrollMin: 1, scrollMax: 1 });
    assert.deepEqual(r.chapter, { equipmentRolls: 3, forgeStoneMin: 8, forgeStoneMax: 12, gemCount: 3, gemLevelMin: 2, gemLevelMax: 3, scrollMin: 1, scrollMax: 2 });
});
